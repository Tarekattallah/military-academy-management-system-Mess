...
async function runTests() {
  try {
    ...
    await Return.deleteMany({ returnNumber: { $regex: 'RET-TEST-P25' } });
    await Batch.deleteMany({ batchNumber: { $regex: 'BATCH-TEST-P25' } });

    // Create batches
    const batch1 = await Batch.create({
      batchNumber: 'BATCH-TEST-P25-1',
      product: product._id,
    ...
    // Helper to create return (to supplier)
    const createRet = async (whId, bId, date) => {
    ...
    console.log('\n--- Phase 2.5: Global Freeze - Return ---');

    // 1. Create on OPEN -> PASS
    let ret1;
    try {
      ret1 = await createRet(warehouse1._id, batch1._id, d1);
      const updatedBatch = await Batch.findById(batch1._id);
      if (updatedBatch && updatedBatch.availableQuantity === 990) {
        console.log('PASSED Case: Create Return in OPEN day (Batch deducted successfully)');
      } else {
        console.error('FAILED Case: Create Return in OPEN day (Regression: Batch not properly deducted)');
      }
    } catch (e) {
      console.error('FAILED Case: Create Return in OPEN day', e);
    }
    ...
    try {
      await createRet(warehouse1._id, batch1._id, d1);
    ...
    if (ret1) {
      try {
        await returnService.cancel(ret1._id, user._id, 'Test Cancellation');
