...
import type { InventoryTransaction, InventoryTransactionFormValues, TransactionType, TransactionModule } from '../../types/inventory';
...
const MODULE_LABELS: Record<TransactionModule, string> = {
  receiving: 'استلام',
  transfers: 'تحويلات',
  returns: 'مرتجعات',
  waste: 'هالك',
...
export function InventoryPage() {
  ...
  const [viewTarget, setViewTarget] = useState<InventoryTransaction | null>(null);
  ...
  const createMutation = useMutation({
    mutationFn: (payload: InventoryTransactionFormValues) => {
    ...
    onError: (err: Error) => toast.error(err.message || 'فشل إنشاء الحركة'),
  });
  ...
  function onSubmit(values: InventoryFormValues) {
    if (!user) return;
    const payload: InventoryTransactionFormValues = {
      batch: values.batch,
  ...
  const columns = [
    {
      ...
      header: 'التاريخ',
      cell: ({ row }: { row: { original: InventoryTransaction } }) =>
    ...
    },
    {
      ...
      header: 'المستودع',
      cell: ({ row }: { row: { original: InventoryTransaction } }) => row.original.warehouse.name,
    },
    {
    ...
    },
    {
      ...
      header: 'بواسطة',
      cell: ({ row }: { row: { original: InventoryTransaction } }) => row.original.performedBy.displayName,
    },
    {
      id: 'actions',
      header: 'إجراءات',
