...
export function ReturnsPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (err) => toast.error(err.message || 'فشل إنشاء الإرجاع')
  });

  const cancelMutation = useMutation({
    mutationFn: ({ id, reason }) => cancelReturn(id, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['returns'] });
    ...
    onError: (err) => toast.error(err.message || 'فشل إلغاء الإرجاع')
  });
  ...
  { accessorKey: 'returnDate', header: 'التاريخ', cell: ({ row }) => new Date(row.original.returnDate).toLocaleDateString('ar-EG') },
  {
    accessorKey: 'status',
    header: 'الحالة',
    cell: ({ row }) =>
