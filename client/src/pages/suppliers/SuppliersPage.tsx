...
export function SuppliersPage() {
  ...
  return (
    <AppLayout title="الموردين">
      ...
      >
        <form id="supplier-form" onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
          <FormField label="الاسم" required error={form.errors.name?.message}>
            <Input {...form.register('name')} placeholder="اسم المورد" />
          </FormField>

          <FormField label="جهة الاتصال" error={form.errors.contactPerson?.message}>
            <Input {...form.register('contactPerson')} placeholder="جهة الاتصال" />
          </FormField>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <FormField label="الهاتف" error={form.errors.phone?.message}>
              <Input {...form.register('phone')} placeholder="رقم الهاتف" dir="ltr" />
            </FormField>

            <FormField label="البريد الإلكتروني" error={form.errors.email?.message}>
              <Input {...form.register('email')} placeholder="البريد الإلكتروني" type="email" />
            </FormField>
          </div>

          <FormField label="العنوان" error={form.errors.address?.message}>
            <Input {...form.register('address')} placeholder="العنوان" />
          </FormField>

          <FormField label="الرقم الضريبي" error={form.errors.taxId?.message}>
            <Input {...form.register('taxId')} placeholder="الرقم الضريبي" />
          </FormField>

          <FormField label="ملاحظات" error={form.errors.notes?.message}>
