...
import type { Waste, WasteStatus } from '../../types/waste';
...
const STATUS_VARIANTS: Record<WasteStatus, 'secondary' | 'success' | 'destructive'> = {
  draft: 'secondary',
  completed: 'success',
...
const emptyForm: WasteFormValues = {
  warehouse: '',
  wasteDate: '',
  reason: '',
  notes: '',
...
export function WastePage() {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: WasteFormValues) => {
  ...
  function onSubmit(values: WasteFormValues) {
    if (!user) return;
  ...
  const columns = [
    { accessorKey: 'wasteNumber', header: 'رقم السجل', cell: ({ row }: { row: { original: Waste } }) => <span className="font-mono text-xs font-medium">{row.original.wasteNumber}</span> },
    { accessorKey: 'warehouse', header: 'المستودع', cell: ({ row }: { row: { original: Waste } }) => row.original.warehouse.name },
    { accessorKey: 'wasteDate', header: 'التاريخ', cell: ({ row }: { row: { original: Waste } }) => new Date(row.original.wasteDate).toLocaleDateString('ar-EG') },
    { accessorKey: 'status', header: 'الحالة', cell: ({ row }: { row: { original: Waste } }) => <Badge variant={STATUS_VARIANTS[row.original.status]}>{STATUS_LABELS[row.original.status]}</Badge> },
    { accessorKey: 'reason', header: 'السبب', cell: ({ row }: { row: { original: Waste } }) => <span className="max-w-[200px] truncate">{row.original.reason}</span> },
    { accessorKey: 'items', header: 'البنود', cell: ({ row }: { row: { original: Waste } }) => <span className="font-mono">{row.original.items.length}</span> },
    { accessorKey: 'createdBy', header: 'بواسطة', cell: ({ row }: { row: { original: Waste } }) => row.original.createdBy.displayName },
    ...
    <AppLayout title="الهالك">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          {canCreate && <Button onClick={handleOpenCreate}><Plus className="size-4" /> تسجيل هالك</Button>}
        </CardHeader>
        <CardContent>
