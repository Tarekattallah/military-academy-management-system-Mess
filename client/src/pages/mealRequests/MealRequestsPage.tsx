...
type MealRequestFormSchema = z.infer<typeof mealRequestSchema>;

const emptyForm = (): MealRequestFormSchema => ({
  requestingUnit: '',
  menu: '',
  notes: '',
...
export function MealRequestsPage() {
  ...
  const [viewTarget, setViewTarget] = useState<MealRequest | null>(null);
  const [editing, setEditing] = useState<MealRequest | null>(null);
  ...
  const createMutation = useMutation({
    ...
    onError: (err: Error) => toast.error(err.message || 'فشل تقديم طلب الوجبات'),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => updateMealRequest(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['meal-requests'] });
      setOpen(false);
      setEditing(null);
    ...
    onError: (err: Error) => toast.error(err.message || 'فشل تحديث الطلب'),
  });
  ...
  const approveMutation = useMutation({
    ...
    onError: (err: Error) => toast.error(err.message || 'فشل الموافقة على الطلب'),
  });

  const rejectMutation = useMutation({
    mutationFn: ({ id, reason }: { id: string; reason: string }) => rejectMealRequest(id, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['meal-requests'] });
      setRejectId(null);
      setRejectReason('');
    ...
    onError: (err: Error) => toast.error(err.message || 'فشل رفض الطلب'),
  });

  const form = useForm<MealRequestFormSchema>({
    resolver: zodResolver(mealRequestSchema) as any,
  ...
  function handleOpenEdit(req: MealRequest) {
    setEditing(req);
    form.reset({
  ...
  function onSubmit(values: MealRequestFormSchema) {
    if (editing) {
  ...
  const columns = [
    {
      ...
      header: 'رقم الطلب',
      cell: ({ row }: { row: { original: MealRequest } }) => (
