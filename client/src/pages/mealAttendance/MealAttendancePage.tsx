...
import type { MealDistribution } from '../../types/mealDistributions';
...
type DistributionFormSchema = z.infer<typeof distributionFormSchema>;
...
type CompleteFormSchema = z.infer<typeof completeSchema>;
...
export function MealAttendancePage() {
  ...
  const [viewTarget, setViewTarget] = useState<MealDistribution | null>(null);
  const [completeTarget, setCompleteTarget] = useState<MealDistribution | null>(null);
  ...
  const createMutation = useMutation({
    ...
    onError: (err: Error) => toast.error(err.message || 'فشل البدء في التوزيع'),
  });

  const completeMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => completeMealDistribution(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['meal-distributions'] });
    ...
    onError: (err: Error) => toast.error(err.message || 'فشل إنهاء التوزيع'),
  });

  const cancelMutation = useMutation({
    mutationFn: ({ id, reason }: { id: string; reason: string }) => cancelMealDistribution(id, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['meal-distributions'] });
      setCancelId(null);
      setCancelReason('');
    ...
    onError: (err: Error) => toast.error(err.message || 'فشل إلغاء التوزيع'),
  });

  const form = useForm<DistributionFormSchema>({
    resolver: zodResolver(distributionFormSchema) as any,
    defaultValues: { reservation: '', notes: '' },
  });

  const completeForm = useForm<CompleteFormSchema>({
  ...
  function handleOpenComplete(dist: MealDistribution) {
    completeForm.reset({
      notes: dist.notes || '',
      items: dist.items.map((item) => ({
        product: item.product?._id || (item.product as any),
        batch: item.batch?._id || (item.batch as any),
  ...
  function onSubmitCreate(values: DistributionFormSchema) {
    createMutation.mutate(values);
  }

  function onSubmitComplete(values: CompleteFormSchema) {
  ...
  const columns = [
    {
      ...
      cell: ({ row }: { row: { original: MealDistribution } }) => (
    ...
    },
    {
      ...
      cell: ({ row }: { row: { original: MealDistribution } }) => row.original.requestingUnit || '—',
    },
    {
