...
type RecipeFormSchema = z.infer<typeof recipeSchema>;

const emptyForm = (): RecipeFormSchema => ({
  recipeNumber: '',
  name: '',
  description: '',
  category: '',
...
export function RecipesPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: Error) => toast.error(err.message || 'فشل إنشاء الوصفة'),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: Error) => toast.error(err.message || 'فشل تحديث الوصفة'),
  });
  ...
  const toggleStatusMutation = useMutation({
    ...
    onError: (err: Error) => toast.error(err.message || 'فشل تحديث حالة الوصفة'),
  });
  ...
  function onSubmit(values: RecipeFormSchema) {
    if (editing) {
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'التصنيف',
      cell: ({ row }: { row: { original: Recipe } }) => row.original.category?.name || '—',
    },
    {
      accessorKey: 'yield',
      header: 'حصص التقديم',
