...
export function ReservationsPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (err) => toast.error(err.message || 'فشل إنشاء الحجز (قد يكون المخزون غير كافٍ)')
  });

  const releaseMutation = useMutation({
    mutationFn: ({ id, notes }) => releaseReservation(id, notes),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['reservations'] });
      setReleaseId(null);
      setReleaseNotes('');
    ...
    onError: (err) => toast.error(err.message || 'فشل تحرير الحجز')
  });

  const consumeMutation = useMutation({
    mutationFn: ({ id, notes }) => consumeReservation(id, notes),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['reservations'] });
      setConsumeId(null);
      setConsumeNotes('');
    ...
    onError: (err) => toast.error(err.message || 'فشل استهلاك الحجز')
  });
