    ...
    switch (reportType) {
      case 'inventory':
        return [
          ...
          { accessorKey: 'inboundQty', header: 'الوارد الكلي', cell: ({ row }: any) => <span className="font-mono text-success">+{row.original.inboundQty || 0}</span> },
          { accessorKey: 'outboundQty', header: 'المنصرف الكلي', cell: ({ row }: any) => <span className="font-mono text-destructive">-{row.original.outboundQty || 0}</span> },
          { accessorKey: 'adjustmentQty', header: 'التسويات', cell: ({ row }: any) => <span className="font-mono">{row.original.adjustmentQty || 0}</span> },
          { accessorKey: 'endingQty', header: 'الرصيد المتبقي', cell: ({ row }: any) => <span className="font-mono font-bold text-primary">{row.original.endingQty || 0}</span> },
        ];
      case 'batches':
        return [
          { accessorKey: 'batchNumber', header: 'رقم الدفعة', cell: ({ row }: any) => <span className="font-mono font-semibold">{row.original.batchNumber}</span> },
          { accessorKey: 'productName', header: 'المنتج' },
          { accessorKey: 'warehouseName', header: 'المستودع' },
          { accessorKey: 'initialQuantity', header: 'الكمية الأصلية', cell: ({ row }: any) => <span className="font-mono">{row.original.initialQuantity}</span> },
          { accessorKey: 'availableQuantity', header: 'المتاح حالياً', cell: ({ row }: any) => <span className="font-mono text-primary font-semibold">{row.original.availableQuantity}</span> },
          { accessorKey: 'status', header: 'الحالة', cell: ({ row }: any) => {
              const statusColors: Record<string, string> = { active: 'success', expired: 'destructive', depleted: 'secondary', near_expiry: 'warning' };
              const statusLabels: Record<string, string> = { active: 'نشط', expired: 'منتهي الصلاحية', depleted: 'مستنفذ', near_expiry: 'قريب الانتهاء' };
          ...
          { accessorKey: 'expiryDate', header: 'تاريخ الصلاحية', cell: ({ row }: any) => row.original.expiryDate ? new Date(row.original.expiryDate).toLocaleDateString('ar-EG') : '-' },
        ];
      case 'receiving':
        return [
          ...
          { accessorKey: 'totalItems', header: 'عدد الأصناف', cell: ({ row }: any) => <span className="font-mono">{row.original.totalItems || 0}</span> },
          { accessorKey: 'totalCost', header: 'التكلفة الإجمالية', cell: ({ row }: any) => <span className="font-mono font-bold text-success">{row.original.totalCost?.toLocaleString('ar-EG')} ر.س</span> },
          { accessorKey: 'receivedDate', header: 'تاريخ الاستلام', cell: ({ row }: any) => new Date(row.original.receivedDate).toLocaleDateString('ar-EG') },
        ];
      case 'transfers':
        return [
          ...
          { accessorKey: 'totalItems', header: 'الأصناف', cell: ({ row }: any) => <span className="font-mono">{row.original.totalItems || 0}</span> },
          { accessorKey: 'status', header: 'الحالة', cell: ({ row }: any) => row.original.status === 'completed' ? 'مكتمل' : 'قيد التحويل' },
          { accessorKey: 'transferDate', header: 'التاريخ', cell: ({ row }: any) => new Date(row.original.transferDate).toLocaleDateString('ar-EG') },
        ];
      case 'wastes':
        return [
          ...
          { accessorKey: 'warehouseName', header: 'المستودع' },
          { accessorKey: 'productName', header: 'المنتج' },
          { accessorKey: 'quantity', header: 'الكمية التالفة', cell: ({ row }: any) => <span className="font-mono text-destructive">{row.original.quantity}</span> },
          { accessorKey: 'reason', header: 'السبب' },
          { accessorKey: 'wasteDate', header: 'التاريخ', cell: ({ row }: any) => new Date(row.original.wasteDate).toLocaleDateString('ar-EG') },
        ];
      case 'reservations':
        return [
          ...
          { accessorKey: 'warehouseName', header: 'المستودع' },
          { accessorKey: 'status', header: 'الحالة', cell: ({ row }: any) => row.original.status },
          { accessorKey: 'totalCost', header: 'القيمة التقديرية', cell: ({ row }: any) => <span className="font-mono">{row.original.totalCost || 0} ر.س</span> },
          { accessorKey: 'reservedDate', header: 'تاريخ الحجز', cell: ({ row }: any) => new Date(row.original.reservedDate).toLocaleDateString('ar-EG') },
        ];
      case 'meal-distributions':
        return [
          ...
          { accessorKey: 'totalItems', header: 'الأصناف المستهلكة', cell: ({ row }: any) => <span className="font-mono">{row.original.totalItems || 0}</span> },
          { accessorKey: 'distributionDate', header: 'التاريخ', cell: ({ row }: any) => new Date(row.original.distributionDate).toLocaleDateString('ar-EG') },
        ];
      case 'consumption':
        return [
          { accessorKey: 'productName', header: 'المنتج الغذائي' },
          { accessorKey: 'totalQuantity', header: 'الكمية المستهلكة الكلية', cell: ({ row }: any) => <span className="font-mono text-primary font-bold">{row.original.totalQuantity || 0}</span> },
          { accessorKey: 'totalCost', header: 'التكلفة الإجمالية', cell: ({ row }: any) => <span className="font-mono text-success">{(row.original.totalCost || 0).toLocaleString('ar-EG')} ر.س</span> },
        ];
      default:
        return [];
    }
